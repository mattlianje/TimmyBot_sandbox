    ...
    if (messageText) {
        ...
        switch (messageText.toLowerCase()) {
            ...
            case 'test clear items':
                ...
                clearItems(friends);
                break;
            case 'test initialize run':
                var friends = getAllID();
                var host = senderID;
                initializeRun(host, friends);
                break;
            case 'test set host':
                var friends = getAllID();
                var host = senderID;
...
// Test fucntion to set current order to empty for all the guys when an order is launched
...
// Test function to set all the hostIDs in hostID field to init host in DynamoDB
...
function initializeRun(hostID, arrayOfAllIDs) {
    for (var i = 0; i < arrayOfAllIDs.length; i++) {
        var table = "run1";
        var currentFriend = arrayOfAllIDs[i];
        console.log("****" + arrayOfAllIDs[i]);
        ...
        var params = {
            ...
            UpdateExpression: "set hostID = :r, orderItem = :p",
            ExpressionAttributeValues: {
                ":r": hostID,
                ":p": [],
            },
